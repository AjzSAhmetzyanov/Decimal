...
clean:
	rm -rf s21_decimal_test test *.gch *.gcda *.gcno *.a *.o *.json coverage.* *.info ./report/ ./s21_decimal_test.dSYM/
all:	build
...
#	valgrind --log-file="log.txt" --tool=memcheck --track-origins=yes --leak-check=yes ./s21_decimal_test
	./s21_decimal_test
...
#	checkmk ./s21_decimal_test.check >s21_decimal_test.c
	$(CC) $(CFLAGS) $(GCOVFLAGS) -c test_decimal.c -o s21_decimal_test.o
