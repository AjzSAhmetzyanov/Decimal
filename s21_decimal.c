...
int s21_mod(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    ...
    if (res.value_type != 0) {
        ...
        if (res.value_type == 2) a = 2;
        if (res.value_type == 3) a = 3;
    }
    return a;
}
    ...
    if (err || scale_res > 28 || scale_res < 0) {
        ...
        while (err && (scale_res <= 28) && (scale_res >= 0)) {
            if (err && (scale_res <= 28) && (scale_res >= 0)) {
                ...
                if (is_equal_b(tmp, zero) ==
                    0) {  // обрезался, нужны идеи как быть в этой ситуации(мб остаток от деления ставить)
                    copy_bits(tmp, big);
                }
 ...
 */
int s21_is_less(s21_decimal dec1, s21_decimal dec2) { return !!s21_is_greater(dec2, dec1); }
 ...
 */
int s21_is_not_equal(s21_decimal value_1, s21_decimal value_2) { return (!s21_is_equal(value_1, value_2)); }
...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
    ...
    if (!dst || code == s21_convert_error) result = s21_convert_error;
    if (get_bit(truncated, 31) || truncated.bits[1] || truncated.bits[2]) result = s21_convert_error;
    int tmp = 0;
    for (int i = 0; i < 31; i++) tmp += pow(2, i) * get_bit(truncated, i);
    if (tmp == INT_MIN && sign) result = s21_convert_error;
    if (sign) tmp = -tmp;
    *dst = tmp;
    return result;
}
...
s21_decimal bit_add(s21_decimal *a, s21_decimal *b, int *error_code) {
    ...
    for (int i = 0; i < 96; i++) {
        ...
        if (!current_bit_a && !current_bit_b) {  // оба бита выключены
            if (buffer) {  // если в буфере что-то есть значит мы повышаем порядок, т.е. прошлый бит 0 , а
                           // этот будет 1
                set_bit(&result, i, 1);
                buffer = 0;
            } else {
...
int zero_check(s21_decimal num1, s21_decimal num2) {
    ...
    if (pt1 && pt2) {
        if (!num1.bits[0] && !num2.bits[0] && !num1.bits[1] && !num2.bits[1] && !num1.bits[2] &&
            !num2.bits[2])
            is_zero = FALSE;
    }
    return is_zero;
 ...
 */
void init_struct(s21_decimal *varPtr) { clear_bits(varPtr); }
void clear_bits(s21_decimal *varPtr) { memset(varPtr->bits, 0, sizeof(varPtr->bits)); }
...
s21_decimal division_without_scale(s21_decimal num1, s21_decimal num2) {
    ...
    int flag_ones = 1;
    for (; i >= 0; i--) {
        if (is_less_b(tmp, div_num2) &&
            !is_equal_b(tmp, div_num2)) {  // для деления столбиком нахожу первое число, которое будет не
            // меньше чем то , на которое делим. дальше делим(вычетанием) tmp
            if (flag_offset == 0) {
                ...
                set_bit(&tmp, 0, get_bit(num1, i));
                if (is_less_b(tmp, div_num2) && !is_equal_b(tmp, div_num2) && i != 0) {
                    flag_null++;
                    continue;
                }
            } else {
...
int is_less_b(s21_decimal num1, s21_decimal num2) { return is_greater(num2, num1); }
int is_greater(s21_decimal num1, s21_decimal num2) {
    ...
    for (int i = 95; i >= 0 && is_greater == -1; i--) {
        ...
        if (is_greater != -1) {
            if (get_sign(&num1) && get_sign(&num2)) is_greater = !is_greater;
        }
    }
    return is_greater;
}
...
int shiftl(s21_decimal *a) {
    ...
    if (b1_tmp) set_bit(a, 32, 1);

    int is_overflow = get_bit(*a, 95);
    a->bits[2] <<= 1;

    if (b2_tmp) set_bit(a, 64, 1);
...
void shiftr(s21_decimal *a) {
    ...
    a->bits[1] >>= 1;
    set_bit(a, 63, 0);
    if (b1_tmp) set_bit(a, 63, 0);

    a->bits[0] >>= 1;

    set_bit(a, 31, 0);

    if (b2_tmp) set_bit(a, 31, 1);
}
...
void bit_on(s22_decimal *a, int position) {
    ...
    if (x < 4) a->bits[x] |= ((unsigned)1 << y);
}

void bit_off(s22_decimal *a, int position) { a->bits[position / 32] &= ~(1 << position % 32); }
...
int s21s22_is_greater_or_equal(s22_decimal dec1, s22_decimal dec2) { return !s21s22_is_less(dec1, dec2); }
